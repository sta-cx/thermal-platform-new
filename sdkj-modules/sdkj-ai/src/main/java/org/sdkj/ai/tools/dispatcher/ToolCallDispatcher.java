...
public class ToolCallDispatcher {
    ...
    public ToolCallResult dispatch(List<ToolCallRequest> calls) {
        ...
        for (ToolCallRequest call : calls) {
            ToolMetadata md = registry.byName(call.toolName());
            if (md == null) {
            ...
            PendingToolCall ptc = PendingToolCall.builder()
                ...
                .effectiveArgs(call.arguments())  // 默认与 arguments 一致,confirm 端点可覆盖
     ...
     * 由于 @Tool 方法的参数名经 -parameters 编译保留,我们用 parameter.getName() 取 map。
     */
