...
public class PrImportHeatController extends BaseController {
    ...
    public R<Void> downloadTemplate(@RequestParam(required = false) String companyId,
        ...
        // TODO: Phase 5c - 下载热表配表导入模板（含已有数据）
        return R.fail("模板下载待实现");
    }

    @SaCheckPermission("thermal:property:import:import")
    @SaCheckLogin
    @Log(title = "热表配表导入", businessType = BusinessType.IMPORT)
    @PostMapping("/import")
    ...
    @SaCheckLogin
    @Log(title = "热表配表导入", businessType = BusinessType.DELETE)
    @DeleteMapping
    ...
    @SaCheckPermission("thermal:property:import:import")
    @SaCheckLogin
    @Log(title = "热表配表提交", businessType = BusinessType.INSERT)
    @PostMapping("/submit")
    public R<Void> submitData() {
        // TODO: Phase 5c - 从临时表提交到 pr_heat_archive
